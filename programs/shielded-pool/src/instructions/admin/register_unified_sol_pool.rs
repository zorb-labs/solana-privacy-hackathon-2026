...
//!
//! Creates a PoolConfigAccount linking to an existing UnifiedSolPoolConfig,
//! enabling the hub to route deposits/withdrawals for unified SOL.
...
use crate::{
    ...
    state::{GlobalConfig, HubPoolType, PoolConfig, UnifiedSolPoolConfig},
};
...
use zorb_pool_interface::{UNIFIED_SOL_POOL_PROGRAM_ID, asset_ids};

/// Accounts for the RegisterUnifiedSolPool instruction.
#[derive(Accounts)]
pub struct RegisterUnifiedSolPoolAccounts<'info> {
    ...
    /// UnifiedSolPoolConfig account from the unified-sol-pool program.
    /// Must be owned by UNIFIED_SOL_POOL_PROGRAM_ID.
    #[account(owner = UNIFIED_SOL_POOL_PROGRAM_ID)]
...
///
/// Creates a PoolConfigAccount linking to an existing UnifiedSolPoolConfig.
/// The asset_id is read from the UnifiedSolPoolConfig (fixed value for all unified SOL).
///
/// # Authority
///
...
/// # Prerequisites
///
/// The UnifiedSolPoolConfig must already exist (created via unified-sol-pool's InitUnifiedSolPoolConfig).
pub fn process_register_unified_sol_pool(
    ctx: Context<RegisterUnifiedSolPoolAccounts>,
) -> ProgramResult {
    let RegisterUnifiedSolPoolAccounts {
        global_config,
        pool_config,
    ...
    if *system_program.key() != pinocchio_contrib::constants::SYSTEM_PROGRAM_ID {
        log!("register_unified_sol_pool: invalid system program");
    ...
    let global_config_bump = global_config.try_map(|config| {
        if config.authority != *authority.key() {
            log!("register_unified_sol_pool: unauthorized");
            return Err(ShieldedPoolError::Unauthorized.into());
        }
        Ok(config.bump)
    })?;

    // Read asset_id from UnifiedSolPoolConfig using type-safe AccountLoader
    let asset_id = unified_sol_pool_config.map(|config| config.asset_id)?;

    // Validate asset_id IS in reserved range (required for unified pools)
    // Unified pools use protocol-assigned asset IDs in the reserved range
    if !asset_ids::is_reserved(&asset_id) {
        log!("register_unified_sol_pool: asset_id not in reserved range");
    ...
    if pool_config.key() != &expected_pda {
        log!("register_unified_sol_pool: invalid pool config PDA");
    ...
    AccountLoader::<PoolConfig>::new(pool_config)?
        .inspect_mut(|config| {
            // Routing (64 bytes)
            config.pool_program = UNIFIED_SOL_POOL_PROGRAM_ID;
            config.asset_id = asset_id;
            // Metadata (8 bytes)
            config.pool_type = HubPoolType::UnifiedSol as u8;
            config.is_active = 1;
            config.bump = bump;
    ...
    let event = PoolRegisteredEvent {
        pool_type: HubPoolType::UnifiedSol as u8,
        _padding: [0u8; 7],
        asset_id,
        pool_program: UNIFIED_SOL_POOL_PROGRAM_ID,
    };
    emit_event(
    ...
    log!("register_unified_sol_pool: success");
